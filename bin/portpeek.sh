...
print_usage() {
  ...
  echo "  -k, --kill     Kill the process using the port (interactive)"
}
...
check_all_ports() {
  echo -e "${YELLOW}Checking all ports...${NC}"
  
  # Try to get all port usage with lsof
  if command_exists lsof; then
    sudo lsof -i -P -n | grep LISTEN
  elif command_exists ss; then
    sudo ss -tulpn
  else
    echo -e "${RED}Error: Neither lsof nor ss command found${NC}"
    exit 1
  fi
}
